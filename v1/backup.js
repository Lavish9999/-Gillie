...
(() => {
  ...
  window.GillieV1?.register("backup", ({ qs, getState, notify, track }) => {
    ...
    group.innerHTML = `
      ...
      <div class="v1-backup-note">Includes streak history, check-ins, cravings, Reef items, and preferences. Gillie Plus is always re-verified by Apple.</div>`;
    ...
    function sanitizeImported(source) {
      ...
      if (!next.cost || typeof next.cost !== "object") next.cost = base.cost || {};
      if (!next.coach || typeof next.coach !== "object") next.coach = base.coach || { missionDate: null, completed: {}, reviews: [] };
      if (!next.reminders || typeof next.reminders !== "object") next.reminders = base.reminders || { checkin: true, craving: true };
